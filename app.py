...
if PERPLEXITY_API_KEY:
    print(f"✅ Found API key in .env file (starts with: {PERPLEXITY_API_KEY[:10]}...)")
else:
    print("❌ PERPLEXITY_API_KEY not found in .env file")

# 2. Check for alternative names
if not PERPLEXITY_API_KEY:
    alternative_keys = ["PPX_API_KEY", "PERPLEXITYAI_API_KEY", "AI_API_KEY"]
...
# 3. Check Streamlit secrets (for Streamlit Cloud)
if not PERPLEXITY_API_KEY:
    try:
        import streamlit as st
...
print(f"Final API key status: {'✅ Available' if PERPLEXITY_API_KEY else '❌ Not available'}")
print("=" * 50)
...
class PerplexityAIHelper:
    """Helper class for Perplexity AI integration using OpenAI SDK."""
    ...
    def __init__(self, api_key=None):
        ...
        if self.api_key:
            try:
                ...
                print(f"❌ Failed to initialize Perplexity AI: {e}")
        else:
            print("❌ No API key provided to PerplexityAIHelper")
...
4. Proper radiology terminology"""
    ...
    def test_connection_public(self):
        ...
        try:
            ...
            if response.choices:
                return response.choices[0].message.content.strip() == "Connected"
            return False
        except Exception as e:
    ...
    if 'initialized' not in st.session_state:
        ...
        if PERPLEXITY_API_KEY:
            ...
            if st.session_state.ai_helper.is_available():
                ...
                print("❌ AI Helper initialization failed")
        else:
            print("❌ No API key available for AI Helper")
...
def main():
                    ...
                    if username in st.session_state.users:
                        if st.session_state.users[username]["password"] == hashed_pw:
                            ...
                            if st.session_state.ai_helper and st.session_state.ai_helper.is_available():
                                ...
                                st.warning(f"⚠️ Welcome, {username}! AI features are unavailable.")
    ...
    if st.session_state.ai_helper and st.session_state.ai_helper.is_available():
        ...
        with st.expander("🔧 Debug Info"):
            st.write(f"API Key Loaded: {'✅ Yes' if PERPLEXITY_API_KEY else '❌ No'}")
            if PERPLEXITY_API_KEY:
                st.write(f"Key starts with: {PERPLEXITY_API_KEY[:10]}...")
            st.write("Check console logs for more details.")
                ...
                if st.button("🔄 Test AI Connection", use_container_width=True,
                    ...
                    if st.session_state.ai_helper and st.session_state.ai_helper.is_available():
                        with st.spinner("Testing connection..."):
                            try:
                                if st.session_state.ai_helper.test_connection_public():
